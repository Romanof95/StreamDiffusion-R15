...
- LoRAs are stacked: the LCM LoRA (when used) and every LoRA of the list are loaded as named adapters and fused in one pass with their weights (`fuse_lora_adapters`), then the PEFT wrappers are dropped. Fusing after each load kept only the last LoRA (loading the next adapter unmerges the fused ones), e.g. Hyper-SDXL + a style LoRA ran without Hyper; engines with two or more fused adapters carry `-f2` in their LoRA signature so the ones built before the fix are rebuilt. An LCM LoRA listed with the LoRAs (`latent-consistency/lcm-lora-sdv1-5`, any name containing `lcm`) replaces the automatic one at its own weight instead of stacking a second LCM on top. Local `.safetensors` paths work as LoRA names (pasted spaces and quotes are stripped).
...
- **Multi-step** (2+ `t_index`, SDXL and SD 1.5): by default the StreamDiffusion stream batch denoises the steps of consecutive frames in one batch-N engine (one frame of latency per extra step). `STREAMDIFFUSION_SEQUENTIAL=1` (`StartStreamDiffusion.bat`) runs the steps sequentially on the current frame in the batch-1 engines instead (`predict_x0_batch` of both pipelines, `use_denoising_batch=False`), with the fixed per-step noise, one StreamV2V cache per step (each step attends to the previous frame at its own noise level, like the batch slots; a single cache shared by all steps injects low-noise features into the first step and breaks the image) and the RCFG residual (`cfg_type=self`) propagated from step to step within the frame: no batch-N engine build, one frame of latency instead of N, the step count can change live, and on SDXL the quantized batch-2 engines, which lose the prompt (nvfp4) or break (mxfp8), are not used; at 1024 on a saturated GPU the stream batch gives no throughput anyway (at 512 on SD 1.5 it does hide part of the extra step, so there the choice is latency against fps). `STREAMDIFFUSION_CN_STEPS` (sequential only) sets the ControlNet per step: `all` (default) on every step; `first` on the first step only (diffusers' `control_guidance_end`: the later steps lose the anchoring and redraw the details, which flickers on video); `reuse` runs it on the first step and feeds the same residuals to the later steps (one ControlNet pass per frame, every step anchored; SD 1.5 at 512 with 4 steps 65 -> 52 ms at the flicker of `all`, SDXL Union at 1024 with 2 steps 134 -> 118 ms), scaled by `STREAMDIFFUSION_CN_REUSE_SCALE` (default 1, below 1 leaves the later steps more freedom). The older `STREAMDIFFUSION_CN_FIRST_STEP_ONLY=1` still means `first`. Use the matching LoRA (`Hyper-SDXL-2steps-lora` with `t_index=[1, 25]`, i.e. t=979/499); two close indices such as `[1, 2]` give a mushy image with any LoRA.
