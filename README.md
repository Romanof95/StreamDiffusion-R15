...
- **Multi-step on SDXL** (2+ `t_index`): by default the StreamDiffusion stream batch denoises the steps of consecutive frames in one batch-N engine (one frame of latency per extra step). `STREAMDIFFUSION_SEQUENTIAL=1` (`StartStreamDiffusion.bat`) runs the steps sequentially on the current frame in the batch-1 engines instead (`StreamDiffusionXL.predict_x0_batch`, `use_denoising_batch=False`), with the fixed per-step noise and the StreamV2V cache frozen on intermediate steps: no batch-N engine build, one frame of latency instead of N, the step count can change live, and the quantized batch-2 engines, which lose the prompt (nvfp4) or break (mxfp8), are not used; at 1024 on a saturated GPU the stream batch gives no throughput anyway. `STREAMDIFFUSION_CN_FIRST_STEP_ONLY=1` (sequential only) applies the ControlNet on the first step only. Use the matching LoRA (`Hyper-SDXL-2steps-lora` with `t_index=[1, 25]`, i.e. t=979/499); two close indices such as `[1, 2]` give a mushy image with any LoRA.
