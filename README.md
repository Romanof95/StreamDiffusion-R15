...
- **Multi-step on SDXL** (2+ `t_index`): the steps run sequentially on the current frame in the batch-1 engines (`StreamDiffusionXL.predict_x0_batch`, `use_denoising_batch=False`), with the fixed per-step noise and the StreamV2V cache frozen on intermediate steps. No batch-N engine build, one frame of latency instead of N, and the step count can change live. The StreamDiffusion stream batch (`STREAMDIFFUSION_DENOISING_BATCH=1`) is kept for reference: at 1024 on a saturated GPU it gives no throughput and its quantized batch-2 engines lose the prompt (nvfp4) or break (mxfp8). `STREAMDIFFUSION_CN_FIRST_STEP_ONLY=1` applies the ControlNet on the first step only. Use the matching LoRA (`Hyper-SDXL-2steps-lora` with `t_index=[1, 25]`, i.e. t=979/499); two close indices such as `[1, 2]` give a mushy image with any LoRA.
