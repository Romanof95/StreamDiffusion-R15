...
class ControlNetManager:
    ...
    def _make_union_calibration_loop(self, names: List[str], control_type_idx, batch: int,
        ...
        pipeline's noisy latents and timesteps, one per denoising-batch slot. Runs under no_grad (not
        ...
        def forward_loop(model_q):
            ...
            with torch.no_grad():
                for _ in range(n_images):
                    ...
                    # Per-slot inputs as the pipeline feeds them (denoising batch): slot k holds
                    # the latent noised to the k-th timestep, paired with that timestep. Feeding
                    # the first timestep to every slot mis-calibrates the later steps (soft,
                    # washed-out 2-step output).
                    x0 = stream.encode_image(img.to(dtype))  # (1,4,h,w) noised at slot 0
                    x0 = (x0 - stream.beta_prod_t_sqrt[0] * stream.init_noise[0:1]) / stream.alpha_prod_t_sqrt[0]
                    n_slots = stream.sub_timesteps_tensor.reshape(-1).shape[0]
                    x_t = torch.cat([
                        stream.add_noise(x0, stream.init_noise[k % stream.init_noise.shape[0]:k % stream.init_noise.shape[0] + 1], k % n_slots)
                        for k in range(batch)
                    ], dim=0).contiguous()
                    t = stream.sub_timesteps_tensor.reshape(-1)
                    t = t[[k % n_slots for k in range(batch)]]
